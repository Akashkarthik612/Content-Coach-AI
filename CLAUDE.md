...
| LLM (supervisor / writer / researcher / angle-expand) | `gemini-3.5-flash`, `thinking_level="low"` — `gemini-2.5-flash` (itself the successor to the deprecated `gemini-2.5-flash-lite`) was also deprecated server-side (404 in `backend/ai/logs/errors.log`) |
| LLM (style analyzer) | `gemini-2.0-flash-lite` (`agents/style_agent.py`) — different major version, deliberately not kept in lockstep with the others |
| LLM resilience | `backend/ai/llm_retry.py` — tenacity exponential backoff on Gemini 429/503/504; `backend/ai/api_key_rotation.py` — falls back to `LANGCHAIN_API_KEY_GEMINI_2` when configured |
...
│   └── alembic/versions/         ← Migrations 0001–0014 (0010: post_publish_log FK CASCADE fix; 0011: linkedin_auth table; 0012: user_profile table; 0013: placeholder — the earlier chat_sessions/chat_messages migration this number belonged to was deleted along with that module, see Design Decisions; 0014: thread_registry table)
│
├── frontend/
...
| — | `user_profile` UI | Not built | Backend-only (API + persistence); `researcher_node` reads it (role/industry/audience) to ground its domain-fit judgment when a profile exists, but there's still no frontend form to create one |
