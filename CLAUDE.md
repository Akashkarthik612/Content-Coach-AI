...
│   │   └── agents/
│   │       ├── supervisor.py            ← `supervisor_node` — binds 5 tools, loops (`_MAX_STEPS=30`), emits `SupervisorClassification{route: Literal["research","write","direct"], direct_answer}` via `model_validate_json()`
...
│   │       ├── researcher.py            ← `researcher_linkedin()` — manual `bind_tools()` loop (`_MAX_TOOL_LOOP_ROUNDS=6`) over `web_search` + `search_vault_posts`, grounded in `user_profile` context when available. Its system prompt (`ResearchPromptBuilder.RESEARCH_SYSTEM`) now asks the LLM to choose per-turn between `MODE: CLARIFY` (a single batched question, used when there's too little to work with) and `MODE: ANGLES` with `REQUEST_TYPE: SINGLE_POST` (5 angles) or `REQUEST_TYPE: SERIES` (N≤5 angles titled `"Part {i} of {N} — {Title}"`, one per part). Every angle block now also carries a `· Lens:` bullet (one of the five named lenses for SINGLE_POST, `"Part {i} of {N}"` for SERIES) — `_STRATEGIC_ANGLE_BLOCK_RE` captures it and `to_wire_dicts()` forwards it as the wire dict's `lens` key, which `ChatPage.jsx`'s multi-angle list renders as the small heading above each angle's title. `_parse_summary()` also strips a leading `MODE: ANGLES\nREQUEST_TYPE: ...` header line so it never leaks into the user-facing summary text. `MODE: CLARIFY` is still **not** handled — a clarify turn still parses to 0 angle blocks and raises `ResearcherDecisionError` (see Known Gaps); this remains open because no UI for it exists anywhere in the product yet. `ResearchArtifactParser` parses the final turn into a mode-agnostic `ResearchArtifact` (`ResearchArtifactItem`s with a generic `attributes` bag, now holding `lens`/`audience`/`provokes_type`/`provokes_reason`); only `"strategic_angles"` has a registered regex/format today (still the single hardcoded mode argument passed into `.parse()`), any other mode raises `ResearcherDecisionError`. `researcher_node()` is the Send-dispatched graph wrapper — adapts the artifact back to the existing `{angles, search_context, summary}` wire shape via `ResearchArtifactParser.to_wire_dicts()`, so the review pipeline/frontend are untouched by the mode-agnostic internals
...
| `researcher_node` | COGNITIVE + TOOL CALLER | `agents/researcher.py` | Send-dispatched with minimal state `{user_id, query}`. Runs its own manual tool loop (`web_search` + `search_vault_posts`, up to 6 rounds, separate from `tool_node`), grounded in `user_profile` when available. Prompt can now ask for `MODE: CLARIFY` or `MODE: ANGLES` (`SINGLE_POST` or `SERIES`); each angle also carries a parsed `lens` label (named lens for `SINGLE_POST`, `"Part {i} of {N}"` for `SERIES`), forwarded through `to_wire_dicts()` into the angle wire dict's `lens` key. `MODE: CLARIFY` is still unhandled — a `CLARIFY` response parses to 0 angles and raises `ResearcherDecisionError` (no product UI exists for it yet). Writes `research_result = {angles, search_context, summary}` |
...
| `steps_taken` | int | supervisor_node | tool-loop budget, capped at `_MAX_STEPS=30`; overrunning forces `route="direct"` |
...
START
  ...
  │
  ▼
supervisor_node ── binds 5 tools; loops (capped at 30 steps); emits
...
| Researcher `MODE: CLARIFY` unhandled | `ResearchPromptBuilder.RESEARCH_SYSTEM` (`agents/researcher.py`) lets the LLM choose `MODE: CLARIFY` when there's too little to work with, but `ResearchArtifactParser`/`researcher_linkedin`'s hardcoded `.parse("strategic_angles", ...)` call were never updated to handle it: a `CLARIFY` turn parses to 0 angle blocks and raises `ResearcherDecisionError` instead of surfacing the clarifying question. `MODE: ANGLES`'s `SINGLE_POST`/`SERIES` split is fully wired now (each angle's `· Lens:` line is parsed and forwarded as the wire dict's `lens` key — see the `researcher.py`/`researcher_node` Directory Map and Node Catalogue entries), so this gap is scoped to `CLARIFY` only. No UI concept for a clarifying question exists anywhere in the product yet, so fixing this needs new graph-routing + frontend work, not attempted here |
