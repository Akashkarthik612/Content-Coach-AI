...
getSessionThreads(sessionId)                                         → {session_id, threads: [{thread_id, user_prompt, status, answer, draft, post_id, angles, actions, summary, approval_status, created_at}]}   // NEW 2026-07-25 — GET /sessions/{id}/threads, powers sidebar rehydration
```
...
**As of 2026-07-25:** the sidebar `chats` list is still local React state (still no `chats.js` API module — each entry is `{id, sessionId, title, time}`), but it's no longer a dead end: `selectChat()` calls `getSessionThreads(chat.sessionId)` and rehydrates `messages` from the backend via `threadToMessages()`, and a `lastSessionId` in `localStorage` restores the most recent chat on a fresh page load. Previously (pre-2026-07-25): chat history was in-memory-only for the page's lifetime with no way to read it back — see the 2026-07-25 changelog entry above for the full fix.
...
`tool_node` (the generic `graph.py` `ToolNode`) is bound to `search_vault_posts`, `get_topic_inventory`, `get_style_memory`, and `get_session_context` — kept in sync with `supervisor_node`'s own `bind_tools()` call (same 4), which is the set that actually needs to match since `tool_node` is the executor of supervisor's tool-calling loop (a latent gap where `get_style_memory` was bound on the LLM but missing from `tool_node` was fixed alongside adding `get_session_context`, 2026-07-25). `researcher_node`/`researcher_linkedin` runs its own separate manual tool loop over `web_search` + `search_vault_posts` — it does not go through `tool_node` at all. Keep this in mind if adding/removing a tool.
