...
### Auth — `/api/auth` (only mounted when `AUTH_PROVIDER=local` — dev/test only)
| Method | Path | Body | Returns | Notes |
|---|---|---|---|---|
| POST | `/register` | `{username, email, password}` | `{user_id, username, email}` | bcrypt hash; 409 if username/email taken |
| POST | `/login` | `{username, password}` | `{user_id, username, email}` | `username` field matches either the `username` OR `email` column |

In prod (`AUTH_PROVIDER=supabase`, the default) there is no backend `/api/auth/*` route — register/login happen client-side via `supabase-js`; the backend only verifies the resulting JWT.
