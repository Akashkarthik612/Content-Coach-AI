...
import {
  ...
  ArrowLeft, Clock, Send, X, ThumbsUp, MessageCircle, Repeat2,
} from 'lucide-react';
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '20px 20px 18px', boxShadow: hover ? '0 22px 48px -26px rgba(20,60,30,.4)' : '0 1px 3px rgba(27,28,20,.05)',
    ...
    >
      ...
      <div style={{ fontFamily: SERIF, fontSize: 21, fontWeight: 500, letterSpacing: '-.015em', color: INK, margin: '8px 0 8px', lineHeight: 1.15 }}>
        {angle.title}
      </div>
      <div style={{ fontSize: 13, lineHeight: 1.6, color: '#7A7C6C', marginBottom: 14 }}>{angle.argument}</div>
      <div style={{ fontSize: 11.5, color: '#B0B2A2', marginBottom: 8 }}>For: {angle.audience}</div>

      {expandedSummary && (
        <div style={{ fontSize: 12.5, lineHeight: 1.6, color: '#3A3C30', background: '#FAF9F3', border: `1px solid ${HAIRLINE}`, borderRadius: 10, padding: 10, marginBottom: 10, animation: 'ccFade .4s ease both' }}>
          {expandedSummary}
        </div>
      )}

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 16 }}>
        <button onClick={() => onExpand(index)} disabled={expanding} style={linkBtn}>
          {expanding ? 'Expanding…' : '→ Expand'}
        </button>
        <button onClick={() => onPick(index)} disabled={picking} style={linkBtn}>
          {picking ? 'Drafting…' : '→ Draft this'}
        </button>
      </div>
    </div>
  );
...
/* ────────────────────────────────────────────────────────────────────────
   One AI / user message
   ──────────────────────────────────────────────────────────────────────── */
function MessageBubble({ msg, onToggleTrace, onPick, onExpandAngle, onNoneFit, onApprove, onDecline, onOpenModify, onModifyTextChange, onCancelModify, onSendModify, onCopy, onOpenWorkspace, onRegenerate }) {
  if (msg.role === 'user') {
    return (
        ...
        {msg.kind === 'angles' && phaseDone && (
          <div style={{ marginTop: 20, animation: 'ccFade .5s ease both' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 14 }}>
              <span style={{ fontFamily: SERIF, fontSize: 21, fontWeight: 500, letterSpacing: '-.015em', color: INK }}>
                {msg.angles.length} possible angle{msg.angles.length === 1 ? '' : 's'}
              </span>
              <span style={{ fontSize: 12, color: MUTED_3, fontWeight: 500 }}>Pick one to draft, or expand for more context</span>
            </div>
    ...
    <aside style={{
      ...
      <div style={{ flex: '0 0 auto', display: 'flex', alignItems: 'center', gap: 4, padding: '14px 12px 12px', justifyContent: open ? 'flex-start' : 'center' }}>
        <button onClick={onToggle} title={open ? 'Collapse sidebar' : 'Expand sidebar'} style={iconBtn}>
          <ArrowLeft size={15} style={{ transform: open ? 'none' : 'rotate(180deg)' }} />
        </button>
        {open && (
          <>
...
/* ────────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────────── */
function WorkspaceView({ ws, onBack, onModeChange, onDocChange, onCmdInput, onCmdKey, onSubmitCmd, onChip, onCopy, onPublish, onHistory, publishing, publishResult, versions, showVersions, userName, professionLine }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'ccFade .4s ease both' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: '0 0 300px', minWidth: 300, display: 'flex', flexDirection: 'column', background: SIDEBAR_BG, borderRight: `1px solid ${HAIRLINE}`, overflow: 'hidden' }}>
          <div style={{ flex: '0 0 auto', padding: '18px 20px 12px' }}>
